import React, { useEffect } from 'react';
import { Link, Navigate } from 'react-router-dom';
import { useAuthStore } from '../lib/authStore';
import { FullLoader } from '../components/DashboardLayout';
import './Landing.css';

export default function Landing() {
  const { user, loading } = useAuthStore();

  useEffect(() => {
    if (loading) return;
    const scSteps = document.querySelectorAll('.sc-step');
    const scPanels = document.querySelectorAll('.sc-content');
    const scDots = document.querySelectorAll('.sc-dot');
    
    if (!scSteps.length) return;

    function setActive(name: string | null) {
      if (!name) return;
      scPanels.forEach(p => p.classList.toggle('active', (p as HTMLElement).dataset.visual === name));
      scDots.forEach(d => d.classList.toggle('active', (d as HTMLElement).dataset.jump === name));
      scSteps.forEach(s => s.classList.toggle('active', (s as HTMLElement).dataset.panel === name));
    }

    const scObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting){
          setActive((entry.target as HTMLElement).dataset.panel || null);
        }
      });
    }, { root: null, rootMargin: '-40% 0px -40% 0px', threshold: 0 });

    scSteps.forEach(step => scObserver.observe(step));

    scDots.forEach(dot => {
      dot.addEventListener('click', () => {
        const target = document.getElementById('sc-step-' + (dot as HTMLElement).dataset.jump);
        if (target) target.scrollIntoView({ behavior: 'smooth', block: 'center' });
      });
    });

    setActive('resume');

    return () => {
      scObserver.disconnect();
    };
  }, [loading]);

  if (loading) {
    return <FullLoader />;
  }

  if (user) {
    return <Navigate to="/dashboard" replace />;
  }

  return (
    <div className="landing-page-root">
      <div className="bg-grid"></div>

      <header>
        <nav>
          <Link to="/" className="logo" style={{ textDecoration: 'none' }}>
            <img className="logo-mark" alt="VersaCareer" src="data:image/png;base64," />VersaCareer
          </Link>
          <div className="nav-cta">
            <button className="theme-toggle" aria-label="Toggle theme">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
            </button>
            <Link className="nav-signin" to="/auth?mode=signin">Sign in</Link>
            <Link className="btn btn-primary btn-sm" to="/auth?mode=signup">Get started <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M5 12h14M13 6l6 6-6 6"/></svg></Link>
          </div>
        </nav>
      </header>

      <main>
        <section className="hero">
          <div className="wrap hero-grid">
            <div>
              <div className="eyebrow">CAREER READINESS PLATFORM</div>
              <h1>Don't find a job. <span className="fade">Become the</span> person companies want to hire.</h1>
              <p className="hero-sub">VersaCareer scans your resume, maps the skill gaps holding you back, builds a week-by-week roadmap to close them, and rehearses you with a mock interview — so you walk in knowing exactly where you stand.</p>
              <div className="hero-actions">
                <Link className="btn btn-primary" to="/auth?mode=signup">Get started free →</Link>
                <Link className="btn btn-ghost" to="/pricing">See pricing</Link>
              </div>
              <div className="trust-row">
                <div className="trust-item">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>
                  3 free resume scans
                </div>
                <div className="trust-item">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>
                  No credit card
                </div>
                <div className="trust-item">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 6L9 17l-5-5"/></svg>
                  1 mock interview included
                </div>
              </div>
            </div>

            <div className="scan-card">
              <div className="scan-card-head">
                <div className="scan-title">RESUME_SCAN.JSON</div>
                <div className="scan-badge">analyzing</div>
              </div>
              <div className="scan-body">
                <div className="ring-wrap">
                  <svg width="108" height="108" viewBox="0 0 108 108">
                    <circle className="ring-bg" cx="54" cy="54" r="46" fill="none" strokeWidth="9"/>
                    <circle className="ring-fg" cx="54" cy="54" r="46" fill="none" strokeWidth="9"/>
                  </svg>
                  <div className="ring-num">84</div>
                </div>
                <div className="scan-lines">
                  <div className="scan-line w1"></div>
                  <div className="scan-line w2"></div>
                  <div className="scan-line w3"></div>
                </div>
              </div>
              <div className="scan-tags">
                <div className="tag tag-ok">✓ ATS format</div>
                <div className="tag tag-gap">△ Missing metrics</div>
                <div className="tag tag-gap">△ SQL — gap</div>
                <div className="tag tag-ok">✓ Strong titles</div>
              </div>
            </div>
          </div>
        </section>

        <div className="wrap"><div className="divider"></div></div>

        <section id="pipeline" style={{ paddingBottom: 0 }}>
          <div className="wrap">
            <div className="section-head">
              <div className="kicker">The path</div>
              <h2>One resume in. One interview-ready candidate out.</h2>
              <p className="section-sub">Scroll to see how each module hands off to the next.</p>
            </div>
          </div>

          <div className="sc-dots">
            <button className="sc-dot active" data-jump="resume" aria-label="Resume Analyzer"></button>
            <button className="sc-dot" data-jump="skillgap" aria-label="Skill Gap"></button>
            <button className="sc-dot" data-jump="roadmap" aria-label="Roadmap"></button>
            <button className="sc-dot" data-jump="interview" aria-label="Mock Interview"></button>
          </div>

          <div className="sc-wrap">
            <div className="sc-left">
              <div className="sc-step" id="sc-step-resume" data-panel="resume">
                <div className="sc-step-kicker"><span className="num">1</span>RESUME ANALYZER</div>
                <h3>Know exactly where your resume stands.</h3>
                <p>Upload your resume and get an instant breakdown across ATS formatting, quantified impact, and role alignment — plus specific line-by-line fixes.</p>
                <div className="sc-points">
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Parses PDF or Word in seconds</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Flags missing metrics and weak phrasing</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Scores against your specific target role</div>
                </div>
                <div className="sc-output"><span className="dot-out"></span>Outputs: score + fix list</div>
                <div className="sc-mobile-visual">
                  <div className="sc-mcard">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">RESUME_SCAN.JSON</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-ring-wrap">
                      <svg width="96" height="96" viewBox="0 0 112 112">
                        <circle className="sc-ring-bg" cx="56" cy="56" r="47" fill="none" strokeWidth="9"/>
                        <circle cx="56" cy="56" r="47" fill="none" strokeWidth="9" stroke="var(--mint)" strokeLinecap="round" strokeDasharray="289" strokeDashoffset="68" transform="rotate(-90 56 56)"/>
                      </svg>
                      <div className="sc-ring-num">84</div>
                    </div>
                    <div className="sc-scan-lines">
                      <div className="sc-scan-line w1"></div>
                      <div className="sc-scan-line w2"></div>
                      <div className="sc-scan-line w3"></div>
                    </div>
                    <div className="sc-tag-row">
                      <div className="sc-tag sc-tag-ok">✓ ATS format</div>
                      <div className="sc-tag sc-tag-gap">△ Missing metrics</div>
                      <div className="sc-tag sc-tag-ok">✓ Strong titles</div>
                    </div>
                  </div>
                </div>
              </div>

              <div className="sc-step" id="sc-step-skillgap" data-panel="skillgap">
                <div className="sc-step-kicker"><span className="num">2</span>SKILL-GAP ANALYSIS</div>
                <h3>See exactly which skills are holding you back.</h3>
                <p>We compare your resume against your target role and rank the gaps by impact, so you know what to fix first.</p>
                <div className="sc-points">
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Technical and soft-skill gaps, side by side</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Ranked by how much each one matters</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Feeds straight into your roadmap</div>
                </div>
                <div className="sc-output"><span className="dot-out"></span>Outputs: prioritized gap list</div>
                <div className="sc-mobile-visual">
                  <div className="sc-mcard">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">SKILL_GAP.JSON</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>SQL</span><span>32% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill warn" style={{ width: '32%' }}></div></div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>Data visualization</span><span>55% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill warn" style={{ width: '55%' }}></div></div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>Stakeholder comms</span><span>71% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill" style={{ width: '71%' }}></div></div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>Project leadership</span><span>48% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill warn" style={{ width: '48%' }}></div></div>
                    </div>
                  </div>
                </div>
              </div>

              <div className="sc-step" id="sc-step-roadmap" data-panel="roadmap">
                <div className="sc-step-kicker"><span className="num">3</span>PERSONALIZED ROADMAP</div>
                <h3>A week-by-week plan to close the gap.</h3>
                <p>Your gaps become a concrete sequence of courses and projects, with progress that syncs across every device you use.</p>
                <div className="sc-points">
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Milestones, not vague advice</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Reprioritizes as you complete steps</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Ends with you interview-ready</div>
                </div>
                <div className="sc-output"><span className="dot-out"></span>Outputs: weekly milestones</div>
                <div className="sc-mobile-visual">
                  <div className="sc-mcard">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">ROADMAP.JSON</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-rm-track"><div className="sc-rm-fill" style={{ width: '42%' }}></div></div>
                    <div className="sc-ms">
                      <div className="sc-ms-check done">✓</div>
                      <div className="sc-ms-name">Rewrite resume for target role</div>
                      <div className="sc-ms-tag">DONE</div>
                    </div>
                    <div className="sc-ms">
                      <div className="sc-ms-check current">2</div>
                      <div className="sc-ms-name">Complete SQL fundamentals course</div>
                      <div className="sc-ms-tag">NOW</div>
                    </div>
                    <div className="sc-ms">
                      <div className="sc-ms-check">3</div>
                      <div className="sc-ms-name muted">Build a data visualization project</div>
                      <div className="sc-ms-tag">NEXT</div>
                    </div>
                    <div className="sc-ms">
                      <div className="sc-ms-check">4</div>
                      <div className="sc-ms-name muted">Practice a mock interview</div>
                      <div className="sc-ms-tag">NEXT</div>
                    </div>
                  </div>
                </div>
              </div>

              <div className="sc-step" id="sc-step-interview" data-panel="interview">
                <div className="sc-step-kicker"><span className="num">4</span>AI MOCK INTERVIEW</div>
                <h3>Rehearse before it counts.</h3>
                <p>Practice real questions for your target role with an AI interviewer, then get scored on clarity, structure, and confidence.</p>
                <div className="sc-points">
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Role-specific question sets</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Full transcript with a scored breakdown</div>
                  <div className="sc-point"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3"><path d="M20 6L9 17l-5-5"/></svg>Retake as many times as you need</div>
                </div>
                <div className="sc-output"><span className="dot-out"></span>Outputs: transcript + score</div>
                <div className="sc-mobile-visual">
                  <div className="sc-mcard">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">MOCK_INTERVIEW.LOG</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-chat ai">Tell me about a time you had to work with incomplete data to make a decision.</div>
                    <div className="sc-chat user">In my last role, I had to forecast Q3 demand with only two months of clean data...</div>
                    <div className="sc-chat ai">Good — can you quantify the impact of that decision?</div>
                    <div className="sc-score-strip">
                      <div className="sc-score-label">Structure &amp; clarity</div>
                      <div className="sc-score-val">78/100</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <div className="sc-right">
              <div className="sc-stage">
                <div className="sc-card">

                  <div className="sc-content active" data-visual="resume">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">RESUME_SCAN.JSON</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-ring-wrap">
                      <svg width="112" height="112" viewBox="0 0 112 112">
                        <circle className="sc-ring-bg" cx="56" cy="56" r="47" fill="none" strokeWidth="9"/>
                        <circle className="sc-ring-fg" cx="56" cy="56" r="47" fill="none" strokeWidth="9"/>
                      </svg>
                      <div className="sc-ring-num">84</div>
                    </div>
                    <div className="sc-scan-lines">
                      <div className="sc-scan-line w1"></div>
                      <div className="sc-scan-line w2"></div>
                      <div className="sc-scan-line w3"></div>
                    </div>
                    <div className="sc-tag-row">
                      <div className="sc-tag sc-tag-ok">✓ ATS format</div>
                      <div className="sc-tag sc-tag-gap">△ Missing metrics</div>
                      <div className="sc-tag sc-tag-ok">✓ Strong titles</div>
                    </div>
                  </div>

                  <div className="sc-content" data-visual="skillgap">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">SKILL_GAP.JSON</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>SQL</span><span>32% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill warn" style={{ '--fill': '32%' } as React.CSSProperties}></div></div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>Data visualization</span><span>55% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill warn" style={{ '--fill': '55%' } as React.CSSProperties}></div></div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>Stakeholder comms</span><span>71% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill" style={{ '--fill': '71%' } as React.CSSProperties}></div></div>
                    </div>
                    <div className="sc-gap-row">
                      <div className="sc-gap-head"><span>Project leadership</span><span>48% match</span></div>
                      <div className="sc-gap-track"><div className="sc-gap-fill warn" style={{ '--fill': '48%' } as React.CSSProperties}></div></div>
                    </div>
                  </div>

                  <div className="sc-content" data-visual="roadmap">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">ROADMAP.JSON</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-rm-track"><div className="sc-rm-fill"></div></div>
                    <div className="sc-ms">
                      <div className="sc-ms-check done">✓</div>
                      <div className="sc-ms-name">Rewrite resume for target role</div>
                      <div className="sc-ms-tag">DONE</div>
                    </div>
                    <div className="sc-ms">
                      <div className="sc-ms-check current">2</div>
                      <div className="sc-ms-name">Complete SQL fundamentals course</div>
                      <div className="sc-ms-tag">NOW</div>
                    </div>
                    <div className="sc-ms">
                      <div className="sc-ms-check">3</div>
                      <div className="sc-ms-name muted">Build a data visualization project</div>
                      <div className="sc-ms-tag">NEXT</div>
                    </div>
                    <div className="sc-ms">
                      <div className="sc-ms-check">4</div>
                      <div className="sc-ms-name muted">Practice a mock interview</div>
                      <div className="sc-ms-tag">NEXT</div>
                    </div>
                  </div>

                  <div className="sc-content" data-visual="interview">
                    <div className="sc-panel-head">
                      <div className="sc-panel-title">MOCK_INTERVIEW.LOG</div>
                      <div className="sc-panel-badge">example</div>
                    </div>
                    <div className="sc-chat ai">Tell me about a time you had to work with incomplete data to make a decision.</div>
                    <div className="sc-chat user">In my last role, I had to forecast Q3 demand with only two months of clean data...</div>
                    <div className="sc-chat ai">Good — can you quantify the impact of that decision?</div>
                    <div className="sc-score-strip">
                      <div className="sc-score-label">Structure &amp; clarity</div>
                      <div className="sc-score-val">78/100</div>
                    </div>
                  </div>

                </div>
              </div>
            </div>

          </div>
        </section>

        <div className="wrap"><div className="divider"></div></div>

        <section>
          <div className="wrap">
            <div className="section-head">
              <div className="kicker">Getting started</div>
              <h2>Ten minutes to your first score.</h2>
            </div>
            <div className="steps">
              <div className="step">
                <div className="step-icon">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                </div>
                <h4>Upload your resume</h4>
                <p>PDF or Word — we parse it in seconds, no formatting cleanup needed on your end.</p>
              </div>
              <div className="step">
                <div className="step-icon">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                </div>
                <h4>See where you stand</h4>
                <p>Your score, your gaps, and how you compare to your target role, all on one screen.</p>
              </div>
              <div className="step">
                <div className="step-icon">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/></svg>
                </div>
                <h4>Work the roadmap</h4>
                <p>Follow the plan, retake the scan, and walk into your mock interview already ahead.</p>
              </div>
            </div>
          </div>
        </section>

        <section id="start">
          <div className="wrap">
            <div className="final-cta">
              <h2>Start with your first resume scan.</h2>
              <p className="section-sub">Free. Takes two minutes. No credit card required.</p>
              <div className="final-actions">
                <Link className="btn btn-primary" to="/auth?mode=signup">Get started free →</Link>
                <Link className="btn btn-ghost" to="/pricing">See pricing</Link>
              </div>
              <div className="fine-print">Already have an account? <Link to="/auth?mode=signin">Sign in →</Link></div>
            </div>
          </div>
        </section>
      </main>

      <footer>
        <div className="wrap">
          <div className="footer-grid">
            <div className="footer-brand">
              <div className="logo"><img className="logo-mark" alt="VersaCareer" src="data:image/png;base64," />VersaCareer</div>
              <p>VersaCareer analyzes your resume, maps your skill gaps, builds your roadmap, and rehearses your interview — one connected path from applicant to hire.</p>
            </div>
            <div className="footer-col">
              <h5>Product</h5>
              <Link to="/upload">Resume Analyzer</Link>
              <Link to="/skill-gap">Skill-Gap Analysis</Link>
              <Link to="/roadmap">Roadmap</Link>
              <Link to="/mentor">Mock Interview</Link>
            </div>
            <div className="footer-col">
              <h5>Company</h5>
              <Link to="#">About</Link>
              <Link to="#">Careers</Link>
              <Link to="#">Press</Link>
            </div>
            <div className="footer-col">
              <h5>Legal</h5>
              <Link to="/privacy">Privacy</Link>
              <Link to="#">Terms</Link>
              <Link to="#">Cookies</Link>
            </div>
          </div>
          <div className="footer-bottom">
            <div>© 2026 VersaCareer. All rights reserved.</div>
            <div>Made for people between jobs and job-ready.</div>
          </div>
        </div>
      </footer>
    </div>
  );
}
